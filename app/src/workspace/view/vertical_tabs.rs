...
fn render_solo_project_header(
    ...
    // Full-bleed band: filled background + top/bottom hairlines only (no side
    // borders, no corner radius) — the Solo-style section-divider look.
    let band = Container::new(content.finish())
        .with_background(internal_colors::fg_overlay_3(theme))
        .with_border(
            Border::new(1.)
                .with_sides(true, false, true, false)
                .with_border_fill(internal_colors::fg_overlay_5(theme)),
        )
        .with_padding(
            Padding::uniform(7.)
                .with_left(GROUP_HORIZONTAL_PADDING)
